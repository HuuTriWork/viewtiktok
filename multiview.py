...
if __name__ == "__main__":
    ...
    os.system("cls" if os.name == "nt" else "clear")
    n = int(input(Fore.CYAN + "📌 Nhập số lượng link cần chạy: "))
    object_ids = []
    for i in range(n):
        link = input(Fore.CYAN + f"🔗 Nhập link {i+1}: ").strip()
        object_ids.append(link)

    index = 0
    while True:
        current_link = object_ids[index]
        print(Fore.MAGENTA + f"\n👉 Đang chạy link {index+1}/{n}: {current_link}")
        status, result = create_order(current_link, api_token)
        if status == 200 and result.get("status") == "success":
            ...
            fail_count += 1
            print(Fore.RED + f"❌ Thất bại! {result}")
        countdown(30)
        index = (index + 1) % n
